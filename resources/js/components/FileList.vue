<template>
  <nav class="flex flex-col h-full py-5">
    <div class="mx-3 md:mx-0 mb-1">
      ...
      <div v-if="LogViewer.assets_outdated"
        ...
        Front-end assets are outdated. To update, please run <code
          class="font-mono px-2 py-1 bg-gray-100 dark:bg-gray-900 rounded">php artisan log-viewer:publish</code>
      </div>
    ...
    <div v-show="fileStore.checkBoxesVisibility">
      <p class="text-sm text-gray-600 dark:text-gray-400">Please select files to delete and confirm or cancel deletion.
      </p>
      <div class="grid grid-flow-col pr-4 mt-2"
        :class="[fileStore.hasFilesChecked ? 'justify-between' : 'justify-end']">
        <button v-show="fileStore.hasFilesChecked" @click.stop="confirmDeleteSelectedFiles" class="button inline-flex">
    ...
    <div id="file-list-container" class="relative h-full overflow-hidden">
      <div class="file-list" @scroll="(event) => fileStore.onScroll(event)">
        <div v-for="folder in fileStore.filteredFolders" :key="folder.identifier" :id="`folder-${folder.identifier}`"
          class="relative folder-container">
          <Menu v-slot="{ open }">
            <div class="folder-item-container" @click="fileStore.toggle(folder)"
              ...
              <div class="file-item group">
                <button class="file-item-info group" @keydown="handleKeyboardFileNavigation">
                  ...
                  <span class="file-icon hidden group-hover:inline-block group-focus:inline-block">
                    <ChevronRightIcon
                      :class="[fileStore.isOpen(folder) ? 'rotate-90' : '', 'transition duration-100']" />
                  </span>
                ...
                <MenuButton as="button"
                  ...
                  :data-toggle-id="folder.identifier" @keydown="handleKeyboardFileSettingsNavigation"
              ...
              <transition leave-active-class="transition ease-in duration-100" leave-from-class="opacity-100 scale-100"
                leave-to-class="opacity-0 scale-90" enter-active-class="transition ease-out duration-100"
                enter-from-class="opacity-0 scale-90" enter-to-class="opacity-100 scale-100">
                <MenuItems static v-show="open" as="div" class="dropdown w-48"
                  ...
                  <div class="py-2">
                    ...
                    <button :class="[active ? 'active' : '']">
                      ...
                      <span
                        v-show="!fileStore.cacheRecentlyCleared[folder.identifier] && !fileStore.clearingCache[folder.identifier]">Clear
                        indices</span>
                      <span
                        v-show="!fileStore.cacheRecentlyCleared[folder.identifier] && fileStore.clearingCache[folder.identifier]">Clearing...</span>
                      <span v-show="fileStore.cacheRecentlyCleared[folder.identifier]" class="text-brand-500">Indices
                        cleared</span>
                    </button>
                    </MenuItem>
                    ...
                    <template v-if="folder.can_delete">
                      ...
                      <button @click.stop="confirmDeleteFolder(folder)"
                        :disabled="fileStore.deleting[folder.identifier]" :class="[active ? 'active' : '']">
          ...
          <div class="folder-files pl-3 ml-1 border-l border-gray-200 dark:border-gray-800"
            ...
            <file-list-item v-for="logFile in (folder.files || [])" :key="logFile.identifier" :log-file="logFile"
